...
/*
    ...
    Parte iterativa: risolvere il problema appena descritto con una funzione iterativa  void F(int* T, int R, 
    int C, int * P, int dimP, int *O).  Il significato dei parametri è quello indicato precedentemente. La funzione F 
    può fare uso di  altre funzioni iterative
*/
...
int match_riga(int *A, int C, int *P, int *dimP){
    int match = 0;
    for(int i = 0; i < C && *dimP > 0; i++){
        //printf("%d == %d ?\n", *(A + i), *P);
        if(*(A + i) == *P){
            match++;
            P++;
...
void F(int *T, int R, int C, int *P, int dimP, int *O){
    int dimensione_P = dimP;
    int *app = P;
    for(int i = 0; i < R; i++){
        O[i] = match_riga((T + (C * i)), C, P, &dimensione_P);
        P += O[i];
        if(dimensione_P <= 0){
            //visto che non mi sembra specificato nel testo nel caso in cui trovo per intero P lo resetto
            dimensione_P = dimP;
            P = app;
        }
        //printf("Dimensione P -> %d\n", dimensione_P);
    }
    return;
}
